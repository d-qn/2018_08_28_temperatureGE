...
if(generateVidAllYears) {
  ...
  outfile <- if(dev) "dev_lastYears_GE.gif" else "anim_EveryYear_GE.gif"
...
annot_text$label <- sapply(annot_text$label, function(lab) paste0(strwrap(lab, 32), sep="", collapse="\n"))
  ...
  ani.width = 620 * 1.4, ani.height = 680 * 1.4
  )
}
...
if(generateVidShift) {
  ...
  df <- table %>% 
    filter(Year <= periods$end[1] | Year >= periods$start[2]) %>% 
    mutate(
  ...
  p <- bg_chart (base_size = fontSize) + 
    ...
    geom_segment(
      ...
      size = fontSize / 2.5, colour = "#b30047", alpha = 0.7
    ) +
    geom_xspline(
      data = df,
      aes(x = Month, y = Temperature, group = Year, colour = colour), 
    ...
    geom_line(
      ...
      size = fontSize / 16, linetype = "dashed"
    ) +
    geom_text(
      data = shiftLabel, hjust = 0.5, vjust = 0, nudge_y = 0.2,
      aes(x = Month, y = y1, label = label), 
      size = fontSize / 3.1, colour = '#2b0818', family = font_tw
    ) +
    geom_text(
      data = timePLabel, 
  ...
  ap <- p + 
    labs(title = "", subtitle = "") + 
    transition_states(
      timeP, transition_length = 200, state_length = 260, wrap = T
    ) +
    enter_fade() +
    exit_fade()
  
  #vid <- animate(ap,renderer = ffmpeg_renderer())
  vid <- animate(ap, width = 620 * 1.6, height = 680 * 1.6, fps = 300, 
                 renderer = ffmpeg_renderer(
                   options = list(pix_fmt = "yuv420p", loop = 0))
  )
  vid %>% save_animation(file = paste0("animShift_GE_", periods$start, ".mp4", collapse = "_"))  
}
